...
async function рrb(msg, mongo_client) { //fake
	if (msg.member.hasPermission("ADMINISTRATOR")) {
		let lang = await database.getValue(mongo_client, msg.channel.id, 'language')
		if (msg.content === '!prb') msg.reply(translation[lang].prb_nothing)
		else msg.reply(translation[lang].prb_something + Math.floor(Math.random()*(100-80)+80)+'%')
	}
}
...
async function egg(msg, mongo_client) {
	...
	if (egged === undefined) msg.channel.send(egger.username + translation[lang].egg_undefined)
		...
		if (egger.id === egged.id) msg.channel.send(egger.username + translation[lang].egg_self + egged.username + ': ' + egg_count)
		else msg.channel.send(egger.username+translation[lang].egg_someone1 + egged.username + translation[lang].egg_someone2 + egged.username+': ' + egg_count)
	}
}
...
async function buy_role(msg, mongo_client) {
	...
	if (role_name === '') msg.reply(translation[lang].buy_undefined)
		...
		if (role === undefined) msg.reply(translation[lang].buy_nonexistent1 + `${'`'}${role_name}${'`'}` + translation[lang].buy_nonexistent2)
		else {
					...
					if ((Object.values(roles_db[i])[0] == role.id)) {
						...
						if (roles_db[i].role_price <= money) {
							...
							try {
								...
								await msg.channel.send(`${msg.member}` + translation[lang].buy_success + `${'`'}${role_name}${'`'}`)
							}
							catch {msg.reply(translation[lang].buy_error)}
						}
...
async function top (msg, mongo_client) {
	...
	else if (people_count == 1) msg.channel.send(`${translation[lang].top_oneperson} ${money_obj[0].name} - ₴${money_obj[0].money}`)
	else {
...
async function buy_lottery(msg, mongo_client) {
	...
	for (let i = 0; i < lottery_tickets.length; i++) {
		if (lottery_tickets[i].user === msg.member.id) msg.reply(`${translation[lang].lottery_exists}${lottery_tickets[i].numbers[0]}, ${lottery_tickets[i].numbers[1]}, ${lottery_tickets[i].numbers[2]}, ${lottery_tickets[i].numbers[3]}, ${lottery_tickets[i].numbers[4]}, ${lottery_tickets[i].numbers[5]}`)
		return
	}
	let numbers = []
	...
	msg.reply(`${translation[lang].lottery_bought}${numbers[0]}, ${numbers[1]}, ${numbers[2]}, ${numbers[3]}, ${numbers[4]}, ${numbers[5]}`)
}
